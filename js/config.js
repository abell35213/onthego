...
const CONFIG = {
    ...
    VIEW_MODE_TRAVEL_LOG: 'travel-log',
...
// Deprecated: call MapKitLoader.init() directly from feature modules.
// Kept as a compatibility wrapper for legacy callers.
async function initMapKit(requestedLibraries = []) {
    console.warn('initMapKit() is deprecated. Use MapKitLoader.init() instead.');
    if (typeof MapKitLoader === 'undefined') {
        console.warn('MapKitLoader is not available.');
        return false;
    }
    return MapKitLoader.init(requestedLibraries);
}
